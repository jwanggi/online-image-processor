(function (imageproc) {
    ...
    function applyBasicOp(inputImage, outputImage) {
        switch (currentBasicOp) {
            ...
            case "unsharp":
                var type = $("#unsharp-type").val();
                var size = parseInt($("#unsharp-kernel-size").val());
                var strength = parseFloat($("#unsharp-strength").val());
                var preview = $("#unsharp-preview").val();
                imageproc.unsharp(inputImage, outputImage, type, size, strength, preview);
                break;
        }
    }
    ...
    imageproc.operation = function (inputImage, outputImage) {
        ...
        if (currentShadeLayerOp != "no-op") {
            ...
            //console.log("enter dither-transparent");
            if (currentShadeLayerOp == "dither" &&
                 ...
                 */
                //console.log("dither-transparent");
                for (var i = 0; i < shadeLayer.data.length; i += 4) {

                    var r = shadeLayer.data[i];
                    var g = shadeLayer.data[i + 1];
                    var b = shadeLayer.data[i + 2];
                    //console.log("(", shadeLayer.data[i], ",", shadeLayer.data[i], ",", shadeLayer.data[i], ")");

                    // Check if the current pixel is white
                    if (r === 255 && g === 255 && b === 255) {
                        // Replace the pixel with the corresponding base layer pixel
                        //console.log("(", baseLayer.data[i], ",", baseLayer.data[i], ",", baseLayer.data[i], ")");
        ...
        if (currentOutlineLayerOp != "no-op") {
            ...
            if (currentOutlineLayerOp == "sobel" &&
                ...
                for (var i = 0; i < outlineLayer.data.length; i += 4) {
                    ...
                    var r = outlineLayer.data[i];
                    var g = outlineLayer.data[i + 1];
                    var b = outlineLayer.data[i + 2];
                    //console.log("(", shadeLayer.data[i], ",", shadeLayer.data[i], ",", shadeLayer.data[i], ")");

                    // Check if the current pixel is edge
                    var edge = ($("#sobel-flip").prop("checked")) ? 255 : 0;
